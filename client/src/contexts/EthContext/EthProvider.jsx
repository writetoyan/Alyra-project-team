  ...
  const init = useCallback(
    async (artifactAyg, artifactStaking, artifactVault, artifactNayg, artifactLPToken, artifactPoolSwap, artifactStakingNFT, artifactNftayg) => {
      if (artifactAyg) {
        ...
        let addressNftayg, addressStakingNFT, contractNftayg, contractStakingNFT;
        try {
        ...
        dispatch({
          ...
          data: { web3, accounts, networkID, 
                  ...
                  contractAyg, contractStaking, contractVault, contractNayg, contractLPToken, contractPoolSwap,
                  addressStaking, addressPoolSwap,artifactNayg, artifactNftayg,
                  artifactStakingNFT, contractNftayg, contractStakingNFT, addressStaking, addressStakingNFT
                }
        });
      }
    }, []);
  ...
  useEffect(() => {
    const tryInit = async () => {
      try {
        ...
        init(artifactAyg, artifactStaking, artifactVault, artifactNayg, artifactLPToken, artifactPoolSwap,artifactNftayg, artifactStaking, artifactStakingNFT);
      } catch (err) {
        console.error(err);
      }
    };
